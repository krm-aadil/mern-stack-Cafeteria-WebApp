    ...
    <>
      ...
      <div className="flex items-center justify-between p-4 bg-red-600 text-white">
        <div className="flex items-center">
          <img src="https://static.vecteezy.com/system/resources/previews/028/597/164/original/restaurant-3d-rendering-icon-illustration-free-png.png" alt="Cafeteria Logo" className="w-12 h-12 mr-4" />
        ...
        <div className="container mx-auto flex flex-col-reverse lg:flex-row items-center justify-between lg:space-x-8">
          <div className="text-center lg:text-left lg:w-1/2">
            <h1 className="text-4xl lg:text-5xl font-bold text-red-600 leading-tight mb-4">
              Satisfy Your Cravings at the <br /> Corporate Cafeteria
            </h1>
          ...
          <div className="lg:w-1/2 flex justify-center mb-8 lg:mb-0">
            <img src="https://static.vecteezy.com/system/resources/previews/036/110/064/original/ai-generated-cute-3d-chef-free-png.png" alt="Chef" className="w-64 h-64 lg:w-80 lg:h-80 object-cover rounded-full shadow-lg" />
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {foodItems.length > 0 ? (
              foodItems.map((foodItem) => (
                <div key={foodItem._id} className="border p-4 rounded-lg shadow-md bg-white hover:shadow-lg transition-shadow duration-300">
                  <img src={`http://localhost:5000${foodItem.image}`} alt={foodItem.name} className="w-full h-48 object-cover rounded-md mb-4" />
                  ...
                  <div className="mt-2">
                    ...
                    <select className="border p-2 rounded-full" onChange={(e) => handleSelectQuantity(foodItem._id, e.target.value)}>
                  ...
                  <div className="mt-2">
                    ...
                    <select className="border p-2 rounded-full" onChange={(e) => handleSelectSize(foodItem._id, e.target.value)}>
                      {foodItem.sizes.map((size) => (
                        <option key={size} value={size}>{size}</option>
                      ))}
                    </select>
                  </div>
                  <button className="mt-4 bg-green-500 text-white px-4 py-2 rounded-full hover:bg-green-600 transition-all" onClick={() => handleAddFood(foodItem)}>
                    Add to Order
                  </button>
                </div>
              ))
        ...
        <div className="text-center my-8">
          <button className="bg-red-600 text-white px-6 py-3 rounded-full hover:bg-red-700 transition-all" onClick={handleOrder}>
